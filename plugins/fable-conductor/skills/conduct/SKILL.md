...
Maximise the expensive model where judgment actually lives — shaping, spec, planning, escalation adjudication, and the final whole-branch review — and spend it nowhere else. Everything mechanical between plan approval and final review runs on opus/sonnet/haiku agents dispatched through Workflow templates, coordinated entirely through durable file contracts, never through agent memory. Sockets, where present, connect conductor sessions only — never workers; no worker agent is granted a messaging tool, and that tool grant is itself the access control. They carry only pointers and wake-ups: every ruling and handoff lands in the stream files *before* any message is sent, and a message with no corresponding file state is ignored, not acted on. Every dispatched agent reads its own brief and appends its own evidence; you read the ledger and the escalations, not the transcripts. The waves are adversarial by construction: an implementer produces, a verifier runs the checks verbatim, a reviewer tries to refute "done," and fix loops are fresh dispatches carrying brief + diff + findings. You re-enter at exactly two seams — escalations and final review. Relationship to fable-mode: fable-mode disciplines how a *single* session works; this skill orchestrates *many* sessions across the model ladder. They stack — run this on a Fable session that also honours the fable-mode gates.
...
- **Opus — emulation mode.** The five-gate discipline is the prosthetic that closes part of the judgment gap: invoke `fable-mode` before conducting (in emulation it is REQUIRED, not recommended — if it is not installed, warn the user and inline its gate rules from `references/weave.md` before proceeding). Compensate at the weak seams: raise `final-audit.js`'s `panelSize` to 5. On escalations, work moves 1–2 at full effort and rule fact disputes yourself; reach for **defer-to-Fable** only under its entry test in `references/escalation.md` — the ruling would set precedent beyond the task, or the deadlock is about design intent rather than fact. An honest deferral beats a weak ruling, but most deadlocks are fact disputes an opus conductor should rule and disclose. The Gate-5 report MUST disclose the conductor tier ("adjudicated and reviewed at opus tier") — never let the report imply Fable-grade review it didn't get.
...
- **`tier`** per task on the ladder `fable → opus → sonnet → haiku`: `judgment` → opus implementer + reviewer for design-heavy, normative, or architectural tasks; `standard` → sonnet implementer + reviewer. The verifier is **always haiku** regardless of tier. You escalate a *task's* tier, never a role's — the checkpoint design is unchanged. Ladder note: the bottom rung is still Haiku 4.5 while the upper rungs have moved a generation, so boundary tasks priced for an older sonnet may sit differently — re-look at tier assignments rather than inheriting them. The verifier stays haiku regardless: its job is mechanical, and a widened spread costs it nothing.
...
**On completion:** update the `stream.md` ledger (status + fix rounds per task) **and, when the stream is tracker-bound, the tracker itself** — for each accepted task, comment the reviewer's verdict on its story and close it, in the same turn the wave lands. The ledger is yours; the tracker is the human's, and it is the only surface they can see from a phone. Batching story closure to the end of the stream (or to Gate 4) leaves a day of work looking like an idle tracker — a real field defect, 2026-08-23. A task that escalated stays open with a comment saying what it is waiting on. Agent gates check contracts; the conductor checks the product — different defect classes. When a wave lands an observable surface, exercise it yourself (run the binary, render the output, count the steps) before composing the next wave. Then route every entry in `escalations[]` to the **Escalation loop** below and move that task's dependents to `blocked`, compose the next dependency-free wave from remaining tasks, and dispatch again.
...
- **brief required H2s (this order):** Goal · Done-check · File scope · Required content · Inputs · Verification commands · Report obligation · Tracker (when tracker-bound) · Out of scope. Flags: `parallel_safe`, `testable`, `tier: standard|judgment`.
- **Tracker-bound streams:** the implementer claims its story and comments once per round; the CONDUCTOR comments the verdict and closes the story as each wave lands — never batched to Phase 5. The tracker is the human's only live view; `stream.md` and `report.md` are yours.
